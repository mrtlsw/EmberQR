...
code = code.replace(/const processingRef = React.useRef\(false\);\s*const pendingProcess = React.useRef\(false\);\s*const latestOptions = React.useRef\(options\);\s*useEffect\(\(\) => \{\s*latestOptions.current = options;\s*\}, \[options\]\);\s*useEffect\(\(\) => \{\s*let active = true;\s*const doUpdate = \(\) => \{[\s\S]*?\}, \[options\]\);/, replacement.trim());
