...
code = code.replace(/const processingRef = React.useRef\(false\);\s*const pendingProcess = React.useRef\(false\);\s*useEffect\(\(\) => \{\s*let active = true;\s*async function processImage\(\)[\s\S]*?\}, \[options.originalImage, options.imageRadius\]\);/, replacement.trim());
